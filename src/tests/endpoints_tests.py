...
def test_start_game_error_player_is_missing():
    ...
    db = next(get_session())
    crud_user.delete_user(db=db, user_id=player_2)
...
def test_leave_game_error_game_not_found():
    ...
    game = get_game_from_player(
        db=db,
        player_id=crud_user.get_active_player_id_from_game(
            db,
            player_owner,
            lobby_test,
        ),
    ...
    response = client.post(f'/game/{lobby_test}/leave', json={'playerId': player_2})
...
def test_create_lobby_error_player_not_found():
    player_owner = create_player('testPlayer')
    db = next(get_session())
    crud_user.delete_user(db=db, user_id=player_owner)
    data_lobby = LobbyCreateSchema(
        lobby_name='room',
        lobby_owner=player_owner,
        min_players=2,
...
def test_get_all_lobbies():
    ...
    response = client.get('/lobby', params={'player_id': player_test_1})
    assert response.status_code == 200
    created_lobbys = [
        test_lobby_1,
        test_lobby_2,
...
def test_join_lobby_error_player_not_found():
    ...
    db = next(get_session())
    crud_user.delete_user(db, user_id=player_test_2)
